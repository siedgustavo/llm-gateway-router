...
app = Server("llm-gateway-router-memory-sse")
...
# Implementation logic matching memory_mcp_server.py
def memory_add(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "created_at": now,
        "updated_at": now,
        "source": "llm-gateway-router-memory",
    }
...
def ensure_collection() -> None:
    try:
        ...
        pass
    logger.info(f"Creating collection {Settings.collection}...")
    qdrant_put(
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting MCP SSE memory server on port {port}...")
